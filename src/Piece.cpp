...
Piece::Piece(Cell pos)
    :pos{pos} {
        srand(time(0));
        type = static_cast<tipo_pieza>(rand() % 7);
...
std::vector<Cell> Piece::next_caer_body() {
...
std::vector<Cell> Piece::next_mover_body(enum move_direction dir) {
    std::vector<Cell> next_body;
    for (const auto &point : body)
...
std::vector<Cell> Piece::next_rotar_body() {
    std::vector<Cell> next_body;
    for (const auto &point : body) {
        int point_x = point.get_x() - pos.get_x();
        int point_y = point.get_y() - pos.get_y();
